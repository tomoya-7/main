...
request1.open('GET', '../audio/hurusato_s.mp3', true);
...
request2.open('GET', '../audio/hurusato_a.mp3', true);
...
request3.open('GET', '../audio/hurusato_t.mp3', true);
...
request4.open('GET', '../audio/hurusato_bs.mp3', true);
...
request5.open('GET', '../audio/hurusato_p.mp3', true);
request5.responseType = 'arraybuffer';
request5.setRequestHeader('Content-Type', 'audio/mp3');
request5.onload = function () {
  audioContext.decodeAudioData(request5.response, function (buffer) {
...
var ch_icon_bs = document.getElementById('ch_icon_bs');
...
var mo_bs = document.getElementById('mo_bs');
...
ch_icon_bs.addEventListener('click', function () {
  const currentValue4 = gainNode4.gain.value;
  if (currentValue4 === 0.5) {
    gainNode4.gain.value = 0;
    ch_icon_bs.classList.add('mute');
    mo_bs.classList.add('mute');
  } else if(currentValue4 === 0) {
    gainNode4.gain.value = 0.5;
    ch_icon_bs.classList.remove('mute');
    mo_bs.classList.remove('mute');
  }
});
ch_icon_p.addEventListener('click', function () {
  const currentValue5 = gainNode5.gain.value;
  if (currentValue5 === 1) {
